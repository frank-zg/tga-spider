...
public class Play implements Runnable {
    ...
    @Override
    public void run() {
        Optional<ProxyIpRepository.IpBean> ipBeanOpl = ProxyIpRepository.takeIp();
        if (!ipBeanOpl.isPresent()) {
            System.out.println("ip group exception...");
            return;
        }

        ProxyIpRepository.IpBean ipBean = ipBeanOpl.get();
        ProxyServer proxy = new ProxyServer.Builder(ipBean.getHost(), ipBean.getPort()).build();
        ...
        kvCommon("guid=4622487A6699E4F92E2A083A12D25E5899B7CE21&iQQ=125826029&ctime=" + cTimeStr() + "&adid=&sdtfrom=70202&surl" +
                ...
                "=3007&vurl=&Pwd=881273072&sid=" + sId, proxy);
        kvCommon("vid=" + sId + "&url=" + uri_index + "&itype=50&str1=&ver=TencentPlayerLiveV3%2E2%2E0%2E00&int2=0&bid" +
                "=pcvideo&iSta=7&val2=&int1=0&str3=&str2=1%2E4%2E6&rnd=750&val=100&str4=8153EF79A1F508FF04C2F546B218549D1E0CA905&iTy=2052", proxy);
        ...
        kvCommon("guid=4622487A6699E4F92E2A083A12D25E5899B7CE21&iQQ=125826029&ctime=" + cTimeStr() +
                ...
                "&P2PVer=0&val1=0&p2pver=0&val2=0&BossId=3007&vurl=" + videoUri + "&Pwd=881273072&sid=" + sId, proxy);
        ...
        kvCommon("guid=4622487A6699E4F92E2A083A12D25E5899B7CE21&ftime=" + timestampMill() +
                "&vkey=" + vKey + "&cnlID=" + sId + "&Pwd=779660211&platform=1&BossId=3460", proxy);
...
//        executorService.execute(() -> {
        ...
        if (videoTime > 0 && videoDownSize > 0) {
            stream = new Thread(() -> httpDownload(proxy));
            stream.start();
        }
        ...
        kvCommon("vid=" + sId + "&url=" + uri_index + "&itype=52&str1=&ver=TencentPlayerLiveV3%2E2%2E0%2E00&int2=0&bid" +
                "=pcvideo&iSta=7&val2=&int1=0&str3=&str2=1%2E4%2E6&rnd=8862&val=100&str4=8153EF79A1F508FF04C2F546B218549D1E0CA905&iTy=2052", proxy);
        ...
        kvCommon("vid=" + sId + "&url=" + uri_index + "&itype=52&str1=&ver=TencentPlayerLiveV3%2E2%2E0%2E00&int2=1&bid" +
                "=pcvideo&iSta=7&val2=&int1=0&str3=&str2=1%2E4%2E6&rnd=5318&val=100&str4=8153EF79A1F508FF04C2F546B218549D1E0CA905&iTy=2052", proxy);
...
//        });
    ...
    public void indexPage(ProxyServer proxy) {
        ...
        HttpUtil.getOrReturn(uri_index, headers1, proxy, exceptionStatus, timeout);
    }

    public void forEachRequest(int seq, ProxyServer proxy) {
        try {
            live_poll(proxy);
            sleep();
            kvCommon("guid=4622487A6699E4F92E2A083A12D25E5899B7CE21&ftime=" + timestampMill() + "&vkey" +
                    "=" + vKey + "&cnlID=" + sId + "&Pwd=779660211&platform=1&BossId=3460", proxy);
            sleep();
            kvCommon("SuperNodePort=0&errorCode=10000&prdLength=60&PeerServerIP=0&downSpeed=111&PeerServerPort=0&app=live&maxSpeed=0" +
                    ...
                    "=0&guid=4622487A6699E4F92E2A083A12D25E5899B7CE21&pla=1&SuperNodeIP=0&fullecode=10000", proxy);
            sleep();
            live_poll(proxy);
            sleep();
            kvCommon("guid=4622487A6699E4F92E2A083A12D25E5899B7CE21&ftime=" + timestampMill() + "&vkey" +
...
//    }
    ...
    public void kvGetCommon(String uri, ProxyServer proxy) {
        Map<String, String> headers = new HashMap<>();
        ...
        HttpUtil.get(uri, headers, proxy, exceptionStatus, timeout);
    }

    public void kvCommon(String body, ProxyServer proxy) {
        Map<String, String> headers = new HashMap<>();
        headers.put("Accept", "*/*");
        headers.put("Accept-Encoding", "gzip, deflate");
        headers.put("Accept-Language", "zh-CN,zh;q=0.8,en;q=0.6");
        ...
        headers.put("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.101 Safari/537.36");
        headers.put("X-Requested-With", "ShockwaveFlash/26.0.0.151");
        HttpUtil.post(KV_URL, body, headers, proxy, exceptionStatus, timeout);
    }

    public void live_poll(ProxyServer proxy) {
        Map<String, String> headers = new HashMap<>();
        headers.put("Accept", "*/*");
        headers.put("Accept-Encoding", "gzip, deflate");
        headers.put("Accept-Language", "zh-CN,zh;q=0.8,en;q=0.6");
        ...
        HttpUtil.get("http://live.mobile.video.qq" +
                ...
                "&markContext=last%3D0", headers, proxy, exceptionStatus, timeout);
    }

    /**
     * http stream下载
     */
    public void httpDownload(ProxyServer proxy) {
        ...
        headers.put("X-Requested-With", "ShockwaveFlash/26.0.0.151");
        HttpUtil.downStream(tmpUrl, headers, proxy, videoDownSize, exceptionStatus, timeout);
    }
}
